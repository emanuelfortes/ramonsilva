...
export default function VagaPage() {
  return (
    <main className="grain relative flex min-h-svh flex-col overflow-hidden bg-bg">
      ...
      {/* Topo: logo + voltar */}
      <header className="relative z-10 mx-auto flex w-full max-w-7xl items-center justify-between px-5 pt-6 sm:px-8 sm:pt-8">
      ...
      <section className="relative z-10 mx-auto flex w-full max-w-7xl flex-1 items-center px-5 py-10 sm:px-8 sm:py-14">
        <div className="w-full lg:max-w-[46%]">
          <p className="flex items-center gap-3 font-label text-lg uppercase tracking-[0.22em] text-fg/90 sm:text-2xl">
            <span className="h-px w-6 bg-gold sm:w-8" aria-hidden="true" />
            {vaga.chapeu}
          </p>

          <h1 className="mt-1 text-5xl font-bold uppercase leading-[0.92] tracking-[-0.02em] text-fg sm:text-7xl lg:text-[5.5rem]">
            {vaga.titulo}
          </h1>

          {/* Placa metálica, como o bloco "BARBEIRO" da arte */}
          <p className="mt-4 inline-block rounded-lg bg-gradient-to-br from-[#e9ddc1] via-[#cdbb92] to-[#9e8d69] px-5 py-2 text-3xl font-bold uppercase tracking-[0.02em] text-[#101010] shadow-[0_10px_40px_-12px_rgba(212,175,55,0.5)] sm:px-7 sm:py-2.5 sm:text-5xl">
            {vaga.cargo}
          </p>

          <ul className="mt-9 space-y-4 sm:mt-12 sm:space-y-5">
            {vaga.requisitos.map((r) => (
              <li key={r.texto} className="flex items-center gap-4">
          ...
          <div className="mt-10 border-t border-line/70 pt-7 sm:mt-14">
